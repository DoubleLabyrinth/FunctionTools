...
void Decryption::DES::Decipher(BYTE srcBytes[8], BYTE K[16][6]) {
    BYTE Buffer[8] = {0};
    ...
    for(BYTE i = 15; i != 255; i--) {
        ...
        //S transformation
        LeftPart[0] = Decryption::DES::S1[(tmp[0] & 0x80) >> 6 | (tmp[0] & 0x04) >> 2][(tmp[0] & 0x78) >> 3] << 4 |
                      Decryption::DES::S2[(tmp[0] & 0x02) | (tmp[1] & 0x10) >> 4][(tmp[0] & 0x01) << 3 | tmp[1] >> 5];
        LeftPart[1] = Decryption::DES::S3[(tmp[1] & 0x08) >> 2 | (tmp[2] & 0x40) >> 6][(tmp[1] & 0x07) << 1 | tmp[2] >> 7] << 4 |
                      Decryption::DES::S4[(tmp[2] & 0x20) >> 4 | (tmp[2] & 0x01)][(tmp[2] & 0x1E) >> 1];
        LeftPart[2] = Decryption::DES::S5[(tmp[3] & 0x80) >> 6 | (tmp[3] & 0x04) >> 2][(tmp[3] & 0x78) >> 3] << 4 |
                      Decryption::DES::S6[(tmp[3] & 0x02) | (tmp[4] & 0x10) >> 4][(tmp[3] & 0x01) << 3 | tmp[4] >> 5];
        LeftPart[3] = Decryption::DES::S7[(tmp[4] & 0x08) >> 2 | (tmp[5] & 0x40) >> 6][(tmp[4] & 0x07) << 1 | tmp[5] >> 7] << 4 |
                      Decryption::DES::S8[(tmp[5] & 0x20) >> 4 | (tmp[5] & 0x01)][(tmp[5] & 0x1E) >> 1];
...
bool Decryption::DES::GenerateK(BYTE CipherKey[8], BYTE out_K[16][6]) {
