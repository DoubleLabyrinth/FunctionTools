...
namespace Math {
    ...
    public:
        ...
        bool operator==(const Matrix<_Tp> srcMatrix) {
            if(RowCount != srcMatrix.RowCount || ColumnCount != srcMatrix.ColumnCount) return false;
            for(UINT32 i = 0; i < RowCount; i++) {
                for(UINT32 j = 0; j < ColumnCount; j++) {
                    if(Data[i][j] != srcMatrix.Data[i][j]) return false;
                }
            }
            return true;
        }
        ...
        bool operator!=(const Matrix<_Tp> srcMatrix) {
            if(RowCount != srcMatrix.RowCount || ColumnCount != srcMatrix.ColumnCount) return true;
            for(UINT32 i = 0; i < RowCount; i++) {
                for(UINT32 j = 0; j < ColumnCount; j++) {
                    if(Data[i][j] != srcMatrix.Data[i][j]) return true;
                }
            }
            return false;
        }
